...
-- RPC to fetch light level summary for multiple users at once
...
BEGIN
  ...
  LEFT JOIN LATERAL (
    ...
    WHERE pl.min_score <= COALESCE(
      (SELECT SUM(fud.daily_light_score) FROM features_user_day fud WHERE fud.user_id = u.uid),
      0
    )
